public class Vigenère {
    ...
    public Vigenère(String pt, String key) {
        this.key = key;
        this.pt = pt.toLowerCase();
    }

    public void keyGenerator(String key) {
        int neededLength = pt.length();
        String neededKey = key;
        String finalNeededKey = "";
        while (neededKey.length() < neededLength) {
            neededKey += neededKey;
        }
        for (int i = 0; i < neededLength; i++) {
            finalNeededKey += neededKey.charAt(i);
        }
